...
@Service
@Slf4j
public class EmbassyKeyService {
    @Value("${embassy.private-key-path}")
    private String privateKeyPath;
    @Value("${embassy.public-key-path}")
    ...
    public void loadKeys() {
        try {
            ...
            log.info("Embassy keys loaded successfully");
        } catch (Exception e) {
            log.error("Failed to load embassy keys: {}", e.getMessage());
        }
    }
} 